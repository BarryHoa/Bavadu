...
async function main() {
  ...
  if (!seedFileName) {
    console.error("Usage: node scripts/seed-file.js <seed-file-name>");
    console.error("Example: node scripts/seed-file.js 0106_seed_shipping_methods.sql");
    process.exit(1);
  }
  ...
  // Tìm file seed trong thư mục migrations
  const migDir = path.join(root, "server/db/migrations");
  const seedFile = path.join(migDir, seedFileName);
  ...
  console.log(`Seeding: ${seedFileName}`);
  
  try {
    await sql.unsafe(sqlText);
    console.log(`✔ Done: ${seedFileName}`);
  } catch (e) {
    console.error(`✖ Failed: ${seedFileName}`);
