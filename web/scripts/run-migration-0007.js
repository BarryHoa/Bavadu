...
async function main() {
  ...
  const migrationFile = path.join(root, "server/db/migrations/0007_create_currencies_table.sql");
  const sqlText = fs.readFileSync(migrationFile, "utf8");

  console.log("Running migration: 0007_create_currencies_table.sql");
  try {
    // Remove comment lines and run the entire SQL
