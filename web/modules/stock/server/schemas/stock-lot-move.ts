...
import { table_stock_move } from "./move";

// Stock Lot Moves - Theo dõi xuất từ lot nào
export const table_stock_lot_move = pgTable(
  "stock_lot_moves",
  {
  ...
  (table) => [
    index("stock_lot_moves_stock_move_idx").on(table.stockMoveId),
    index("stock_lot_moves_stock_lot_idx").on(table.stockLotId),
    index("stock_lot_moves_type_idx").on(table.moveType),
  ]
);
