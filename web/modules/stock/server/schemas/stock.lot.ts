...
export const stock_tb_stock_lots = mdlStockSchema.table(
  ...
  (table) => [
    ...
    index("lots_purchase_order_line_idx").on(table.purchaseOrderLineId),
    ...
    index("lots_variant_status_idx").on(table.productVariantId, table.status),
    // Composite index for warehouse and status
    index("lots_warehouse_status_idx").on(table.warehouseId, table.status),
  ],
);
