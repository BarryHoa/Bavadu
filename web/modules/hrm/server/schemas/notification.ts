...
export const table_notification = pgTable(
  "hrm_notifications",
  {
    id: uuid("id")
      .primaryKey()
  ...
  (table) => [
    index("hrm_notifications_employee_idx").on(table.employeeId),
    index("hrm_notifications_type_idx").on(table.type),
    index("hrm_notifications_read_idx").on(table.isRead),
    index("hrm_notifications_employee_read_idx").on(table.employeeId, table.isRead),
  ]
);
