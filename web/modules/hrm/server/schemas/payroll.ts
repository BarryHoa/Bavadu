...
import { table_payroll_period } from "./payroll-period";

// Payroll - Bảng lương
export const table_payroll = pgTable(
  "hrm_payrolls",
  {
    id: uuid("id")
      .primaryKey()
  ...
  (table) => [
    index("hrm_payrolls_period_idx").on(table.payrollPeriodId),
    index("hrm_payrolls_employee_idx").on(table.employeeId),
    index("hrm_payrolls_period_employee_idx").on(
      table.payrollPeriodId,
      table.employeeId
    ),
    index("hrm_payrolls_status_idx").on(table.status),
  ]
);
