...
// Courses - Khóa học
export const table_course = pgTable(
  "hrm_courses",
  {
    id: uuid("id")
      .primaryKey()
  ...
  (table) => [
    index("hrm_courses_code_idx").on(table.code),
    index("hrm_courses_category_idx").on(table.category),
    index("hrm_courses_active_idx").on(table.isActive),
  ]
);
