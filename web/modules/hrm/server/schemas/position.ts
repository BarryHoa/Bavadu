...
export const table_position = pgTable(
  "hrm_positions",
  {
    id: uuid("id")
      .primaryKey()
  ...
  (table) => [
    ...
    index("hrm_positions_department_idx").on(table.departmentId),
    index("hrm_positions_reports_to_idx").on(table.reportsTo),
    index("hrm_positions_active_idx").on(table.isActive),
  ]
);
