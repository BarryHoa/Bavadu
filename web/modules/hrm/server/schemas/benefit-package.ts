...
export const table_benefit_package = pgTable(
  "hrm_benefit_packages",
  {
    id: uuid("id")
      .primaryKey()
  ...
  (table) => [
    index("hrm_benefit_packages_code_idx").on(table.code),
    index("hrm_benefit_packages_type_idx").on(table.benefitType),
    index("hrm_benefit_packages_active_idx").on(table.isActive),
  ]
);
