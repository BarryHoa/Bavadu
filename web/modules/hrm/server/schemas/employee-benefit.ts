...
import { table_benefit_package } from "./benefit-package";
import { table_employee } from "./employee";

// Employee Benefits - Phúc lợi của nhân viên
export const table_employee_benefit = pgTable(
  ...
  (table) => [
    index("hrm_employee_benefits_employee_idx").on(table.employeeId),
    index("hrm_employee_benefits_package_idx").on(table.benefitPackageId),
    index("hrm_employee_benefits_status_idx").on(table.status),
    index("hrm_employee_benefits_dates_idx").on(table.effectiveDate, table.expiryDate),
  ]
);
