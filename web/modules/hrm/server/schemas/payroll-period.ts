...
export const table_payroll_period = pgTable(
  "hrm_payroll_periods",
  {
    id: uuid("id")
      .primaryKey()
  ...
  (table) => [
    index("hrm_payroll_periods_code_idx").on(table.code),
    index("hrm_payroll_periods_dates_idx").on(table.startDate, table.endDate),
    index("hrm_payroll_periods_status_idx").on(table.status),
  ]
);
