...
export default function EmployeeViewPage(): React.ReactNode {
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        ...
        <div className="flex gap-2">
          ...
          <Button
            size="sm"
            color="primary"
            onPress={() => router.push(`/workspace/modules/hrm/employees/edit/${id}`)}
          >
            {t("edit")}
          </Button>
        </div>
      ...
      <Card>
        <CardBody className="p-4">
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <div>
              <div className="text-sm text-default-500">{t("employeeCode")}</div>
              <div className="text-base">{employee.employeeCode}</div>
            </div>
            <div>
              <div className="text-sm text-default-500">{t("fullName")}</div>
              <div className="text-base">{getLocalizedText(employee.fullName as any)}</div>
            </div>
            <div>
      ...
      <Card>
        <CardBody className="p-4">
          <h2 className="text-base font-semibold mb-4">{t("employmentInfo")}</h2>
          <div className="grid gap-4 md:grid-cols-2">
            <div>
              <div className="text-sm text-default-500">{t("department")}</div>
              <div className="text-base">{getLocalizedText(employee.department?.name as any) || "—"}</div>
            </div>
            <div>
              <div className="text-sm text-default-500">{t("position")}</div>
              <div className="text-base">{getLocalizedText(employee.position?.name as any) || "—"}</div>
            </div>
            <div>
            ...
            <div>
              <div className="text-sm text-default-500">{t("employmentStatus")}</div>
              <div className="text-base">{employee.employmentStatus || "—"}</div>
            </div>
          </div>
        </CardBody>
      </Card>
