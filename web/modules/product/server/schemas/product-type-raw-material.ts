...
export const table_product_type_raw_material = pgTable(
  ...
  (table) => [
    index("product_type_raw_material_variant_idx").on(table.productVariantId),
    index("product_type_raw_material_supplier_idx").on(
      table.primarySupplierId
    ),
    // Composite index for supplier and reorder point queries
    index("product_type_raw_material_supplier_reorder_idx").on(
      table.primarySupplierId,
