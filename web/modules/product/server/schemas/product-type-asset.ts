...
export const table_product_type_asset = pgTable(
  "product_type_asset",
  {
    id: uuid("id")
      .primaryKey()
  ...
  (table) => [
    index("product_type_asset_variant_idx").on(table.productVariantId),
    index("product_type_asset_code_idx").on(table.assetCode),
    index("product_type_asset_assigned_idx").on(table.assignedToUserId),
    // Composite index for assigned assets query
    index("product_type_asset_assigned_location_idx").on(
